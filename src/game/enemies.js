...
/**
 * Πολλαπλασιαστής ταχύτητας του κύματος. Ανεβαίνει και ΣΤΑΜΑΤΑΕΙ: η πίεση
 * υπάρχει για ένταση, όχι για να τιμωρεί την αργή σκέψη.
 *
 * 11/09 (Κοσμάς): «όσο δυσκολεύει να έρχονται και λίγο πιο γρήγορα γιατί
 * νομίζω είναι εύκολο». Ήταν +5% ανά κύμα ως ×1,4 — το ταβάνι ερχόταν ήδη
 * στον 3ο σταθμό και από εκεί και πέρα τίποτα δεν άλλαζε. Τώρα +7% ανά κύμα
 * ως ×2,2: στο κάστρο του πρώτου Δρόμου ~×1,9, στον δεύτερο Δρόμο το ταβάνι.
 * Στο ×2,2 ο γρηγορότερος (σκιερός νίντζα) θέλει ακόμα ~8 δευτ. να φτάσει.
 * @param {number} wave 1-based
 */
export function waveSpeed(wave) {
  return Math.min(1 + Math.max(0, waveIndex(wave)) * .07, 2.2);
}
