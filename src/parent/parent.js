...
function progressHTML(p) {
  ...
  const d = p.profile.daily;
  const today = d && d.ids ? d : null;
  const dailyLine = today ? `<p class="pm-stat">Δεκαπεντάδα της μέρας: <b>${today.correct.filter((id) => today.ids.includes(id)).length}</b>/${today.ids.length} σωστές · σερί <b>${today.streak || 0}</b> (καλύτερο ${today.best || 0}) · ζώνη: <b>${esc(TXT.belts[journey.beltOf(p)])}</b> (${journey.learnedWords(p)} λέξεις)</p>` : '';
  return `${dailyLine}
    ...
    <p class="pm-note">Κάθε μέρα έχει 15 λέξεις: όσες δεν έχει κατακτήσει ακόμα και νέες από τη λίστα ως τις 15. Οι κατακτημένες βγαίνουν από τη δεκαπεντάδα και ξανάρχονται πού και πού ως επανάληψη. Οι κατακτημένες ξανάρχονται μόνες τους σε 3, 7, 14 και 30 μέρες — και ανάμεσα, στην προπόνηση. Καμία δεν αποσύρεται.</p>`;
}
...
function renderMain(note = '') {
  ...
  const w = screen(`
    ...
    <details class="pm-bulk">
      ...
      <p class="pm-note">Βάλε όσες θέλεις — π.χ. τη λίστα της εβδομάδας. Το παιχνίδι τις φέρνει σιγά σιγά (15 λέξεις τη μέρα: όσες δεν έμαθε ακόμα + νέες) και ξαναρωτά πού και πού τις παλιές.</p>
    </details>
