...
//   isPractice: bool         // true = «προπόνηση»: δεν αλλάζει επίπεδο· λάθος φέρνει
//                            // την κανονική επανάληψη το αργότερο αύριο (§6, 11/09)
// }
// type 'intro' = τελετή περγαμηνής (πρώτη έκθεση, χωρίς δυνατότητα λάθους).
//
// Προαιρετικό { intro } — ΠΟΤΕ επιτρέπεται τελετή (προσθήκη 10/09, NEXT-FIXES Δ1).
// Δεν είναι τρίτη κλήση· είναι επιλογή της πρώτης:
//   'gated' (προεπιλογή) → μόνο όσο οι ληξιπρόθεσμοι < intro_when_due_below
//   'first' → ΠΡΩΤΑ όλοι οι νέοι στόχοι, ομαδοποιημένοι ανά λέξη (άνοιγμα λεβελ)
//   'never' → ποτέ τελετή (μέση λεβελ· οι νέοι περιμένουν τον επόμενο πάπυρο)
...
Φωνήεντα: `/i/` ι η υ ει οι · `/o/` ο ω · `/e/` ε αι · `/u/` ου.
Διαλυτικά: `au` αυ/αϋ · `eu` ευ/εϋ · `oi_dia` οϊ/οι · `ei_dia` εϊ/ει ·
`ai_dia` αϊ/αι.
Διπλά σύμφωνα: `ll` `mm` `nn` `pp` `rr` `ss` `tt` `kk` `bb` (απλό/διπλό)
και `gg` γγ/γκ.

**Η ΣΕΙΡΑ ΔΗΛΩΣΗΣ ΜΕΤΡΑΕΙ.** Το `classForGrapheme` επιστρέφει την πρώτη
κλάση που ταιριάζει, οπότε οι κλάσεις των διαλυτικών δηλώνονται μετά τα
φωνήεντα: έτσι το «οι» μένει ερώτημα /i/ και μόνο το «οϊ» ρωτά για
διαλυτικά.

Ο ίδιος πίνακας δίνει και τον τεμαχισμό λέξης σε γραφήματα για τη
συναρμολόγηση (δίψηφα και διπλά σύμφωνα μένουν ενωμένα, ο τόνος μένει πάνω
στο γράμμα του). Ο τονισμός δεν εξασκείται (εκτός scope).

`AUTO_CLASSES` ορίζει ποιες κλάσεις προεπιλέγονται στο Parent Mode: μόνο
φωνήεντα και διαλυτικά. Τα διπλά σύμφωνα τα διαλέγει ρητά ο γονιός.

**Υποψήφια διαφορετικού μήκους:** ένα κενό δέχεται «ν» ή «νν». Επειδή το
πλάτος του κενού θα πρόδιδε το μήκος, το `layoutWord` το υπολογίζει από το
**φαρδύτερο υποψήφιο**, όχι από τον στόχο, και ζωγραφίζει τον στόχο
κεντραρισμένο μέσα στην υποδοχή.
...
```json
{
  ...
  "active_set": { "size_min": 8, "size_max": 12, "current_ratio": 0.6,
                  "current_word_age_days": 21, "intro_when_due_below": 3,
                  "learning_cap_targets": 24 },
  "daily_words": 15, "review_per_day": 3,
...
### Η λίστα που μεγαλώνει (11/09/2026 — ανάθεση ιδιοκτήτη)

> «Δεν θα του βάλω 10 λέξεις και τέλος… την άλλη εβδομάδα θα έχει άλλες
> 10-20 και πρέπει να μαθαίνει τις λέξεις αλλά να γυρνάει και στις παλιές.»

Ο κύκλος ζωής μιας λέξης:

| Φάση | Τι σημαίνει | Τι κάνει το παιχνίδι |
|---|---|---|
| **Περιμένει σειρά** | μπήκε από τον γονιό, δεν έχει παρουσιαστεί | τίποτα ακόμα — μπαίνει με τη σειρά που προστέθηκε |
| **Μαθαίνει** | παρουσιάστηκε στον πάπυρο, επίπεδο < `mastery_level` | ρωτιέται συχνά (0 → 1 μέρα) |
| **Κατακτημένη** | σωστό σε δύο διαφορετικές μέρες | ξαναρωτιέται σε 3, 7, 14, 30 μέρες — **ποτέ δεν αποσύρεται** |

Κανόνες:
- **Όριο μάθησης** (`learning_cap_targets`, 12 σημεία): νέα λέξη παρουσιάζεται
  μόνο όσο τα σημεία που **χρωστάει τώρα** είναι λιγότερα — αναπάντητα (επίπεδο 0)
  ή ληξιπρόθεσμα (`learningLoad`, 18/09). Σημείο που απαντήθηκε σωστά και περιμένει
  το αύριο δεν πιάνει θέση· πριν έπιανε, και η λίστα πάγωνε σε ~6 λέξεις ως την
  επόμενη μέρα (NEXT-FIXES Η5). Δεύτερο φρένο: `new_words_per_day` (15). Λέξη που
  άρχισε να παρουσιάζεται τελειώνει (ποτέ μισή). Ισχύει και για τον πάπυρο
  (`intro:'first'`) και για το ενεργό σύνολο — 30 νέες λέξεις δεν πνίγουν πια
  το session. Όριο στον πάπυρο: και τα 5 της σκηνής (WORDS_PER_LEVEL).
- **Δεκαπεντάδα της μέρας** (v3.24, NEXT-FIXES Θ3 — `learning/daily.js`): κάθε μέρα
  15 λέξεις = όσες δεν κατακτήθηκαν + νέες με τη σειρά της λίστας. Οι κατακτημένες
  βγαίνουν και ξανάρχονται μόνο ως επανάληψη (`review_per_day`, ληξιπρόθεσμες). Το
  engine ρωτά ΜΟΝΟ από αυτές· όταν κλείσει ο στόχος (όλες σωστές μία φορά) → σερί,
  και από εκεί όλες όσες έχει δει, χωρίς νέες. Το σερί ΔΕΝ τιμωρεί (§8.5): χαμένη
  μέρα = ξαναρχίζει, το «καλύτερο» μένει, κανένα μήνυμα.
- **Προπόνηση = γυρισμός στις παλιές**: όταν δεν χρωστάει τίποτα, πρώτα οι
  κατακτημένες και ανάμεσά τους αυτή που έχει να τη δει τον περισσότερο
  καιρό (`lastSeenAt`). Κάθε προπόνηση την κάνει πρόσφατη → η σειρά περνά
  από όλη τη λίστα.
- **Λάθος σε προπόνηση** (αλλαγή της απόφασης «η προπόνηση δεν αγγίζει
  τίποτα»): το επίπεδο μένει, αλλά η κανονική επανάληψη έρχεται το αργότερο
  **αύριο**. Όχι αμέσως — μετά την αποκάλυψη θα ήταν αντιγραφή.
- Ο γονιός βλέπει τη ροή στο Parent Mode (`telemetry.learningFlow`):
  κατακτημένες · μαθαίνει τώρα · περιμένουν σειρά · για επανάληψη σήμερα ·
  νέες των 7 ημερών.
